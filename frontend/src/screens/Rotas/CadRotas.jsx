...
class CadRotas extends React.Component {
  ...
  onRightClickMap() {
    ...
    DirectionsHelper.getRouteAPI(waypoints, result => {
      const mapsRoute = result.routes[0].overview_path.map(point => {
  ...
  saveRoute() {
    ...
    if (minPessoas <= 0 || maxPessoas <= 0) {
      alert(
        "A quantidade estimada de pessoas não pode ser zero ou abaixo de zero"
      );
      canSave = false;
    }
    ...
    if (minPessoas > maxPessoas) {
      alert(
        "A quantidade máxima estimada de pessoas não pode ser menor que a minima"
      );
      canSave = false;
    }
